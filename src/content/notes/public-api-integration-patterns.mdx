...
이 한 줄을 boundary에 박아 두니 schema가 바뀌면 다음 cron에서 곧장 alert가 떴다. 조용히 틀린 값이 흘러 들어가던 자리가 시끄럽게 멈추는 자리로 바뀐 것이다. 덤으로 `any`의 전염이 끊겼고, string→number 같은 transform도 한곳에 모였다. `as` 캐스팅으로 넘겼다면 영영 못 잡았을 일이다.
